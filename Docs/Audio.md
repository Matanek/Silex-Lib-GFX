...
`Audio.Plugin` to make `Audio` available from `startup` until `shutdown`.
...
func start(audio:&Audio) {
    let explosion = audio.load("Assets/Audio/Explosion.wav")
    let playing = audio.play(explosion, Audio.Playback(
        volume:0.8,
        repeat:Audio.Repeat.once(),
...
- `Audio.Repeat.once()` plays once;
- `Audio.Repeat.times(3)` plays three times in total;
- `Audio.Repeat.forever()` repeats until stopped.

`fade_in_seconds` belongs to the initial `Audio.Playback` request. `stop`
...
```sx
let playing = audio.play(ambience, Audio.Playback(
